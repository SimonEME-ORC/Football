...
class Notifications(commands.Cog):
	...
	async def on_member_unban(self, guild, user):
		try:
			c = guild.get_channel(self.bot.notif_cache[guild.id]["unbans_channel_id"])
	...
	async def on_member_join(self, new_member):
		...
		if new_member.id == 272722118192529409:
			try:
				...
				pass
		try:
			j = new_member.guild.get_channel(self.bot.notif_cache[new_member.guild.id]["join_channel_id"])
	...
	async def on_member_remove(self, member):
		# Check if in mod.
		try:
			lc = member.guild.get_channel(self.bot.notif_cache[member.guild.id]["leaves_channel_id"])
					...
					if x.action == discord.AuditLogAction.kick:
						if x.reason is not None:
							if x.reason in ["roulete", "Asked to be"]:
								return
						try:
							kc = member.guild.get_channel(self.bot.notif_cache[member.guild.id]["kicks_channel_id"])
						except (AttributeError, TypeError):
							return
						return await kc.send(f"👢 {member.mention} was kicked by {x.user} for {x.reason}.")
					elif x.action == discord.AuditLogAction.ban:
						try:
							bc = member.guild.get_channel(self.bot.notif_cache[member.guild.id]["bans_channel_id"])
						except (AttributeError, TypeError):
							return
	...
	async def on_guild_emojis_update(self, guild, before, after):
		try:
			c = guild.get_channel(self.bot.notif_cache[guild.id]["emoji_channel_id"])
		except (KeyError, AttributeError):
			return
